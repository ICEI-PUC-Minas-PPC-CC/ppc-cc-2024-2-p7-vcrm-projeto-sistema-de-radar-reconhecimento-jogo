...
function vRPbm.loadFreeze(notify,god,ghost)
	if not frozen then
	  ...
	  if notify then
	    vRP.notify({"You've been unfrozen."})
	  end
	  unfrozen = true
	  invincible = false
	  invisible = false
...
-- vrp_attitudes
-- MOVEMENT CLIPSET
function vRPbm.playMovement(clipset,blur,drunk,fade,clear)
  --request anim
...
function vRPbm.clearAreaOfVehicles(radius)
  local closeby = vRPbm.getNearestVehicle(radius)
  if IsEntityAVehicle(closeby) then -- precheck if vehicle is undriveable
	...
	return vRPbm.clearAreaOfVehicles(radius)
  end
  return true
end

function vRPbm.resetMovement(fade)
  -- reset all
	...
	Citizen.CreateThread(function()
		...
		if DoesEntityExist(vehicleHandle) then
		  if GetVehicleDoorsLockedForPlayer(vehicleHandle,PlayerId()) or any then
			...
			vRP.notify({"Vehicle already unlocked."})
		  end
		else
			vRP.notify({"Too far away from vehicle."})
		end
	end)
end
...
function vRPbm.spawnVehicle(model) 
    ...
    while not HasModelLoaded(mhash) and i < 1000 do
	  if math.fmod(i,100) == 0 then
	    vRP.notify({"Loading vehicle model."})
	  end
      RequestModel(mhash)
      Citizen.Wait(30)
	  i = i + 1
    ...
    if HasModelLoaded(mhash) then
      ...
      SetVehicleNumberPlateText(spawned_car, "ABC"..plate)
      SetVehicleIsStolen(nveh, false)
      SetPedIntoVehicle(GetPlayerPed(-1),nveh,-1) -- put player inside
      Citizen.InvokeNative(0xAD738C3085FE7E11, nveh, true, true) -- set as mission entity
      SetVehicleHasBeenOwnedByPlayer(nveh,true)
      SetModelAsNoLongerNeeded(mhash)
	  vRP.notify({"Vehicle spawned."})
	else
	  vRP.notify({"Vehicle model invalid."})
	end
end
		...
		if IsPlayerPlaying(PlayerId()) and state_ready then
			if vRPbm.getArmour() == 0 then
				...
				if(GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01")) then
					SetPedComponentVariation(GetPlayerPed(-1), 9, 4, 1, 2)  --Bulletproof Vest
				end
