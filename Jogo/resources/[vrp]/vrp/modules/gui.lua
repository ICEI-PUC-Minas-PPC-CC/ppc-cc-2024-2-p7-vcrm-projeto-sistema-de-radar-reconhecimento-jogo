...
function vRP.openMainMenu(source)
  vRP.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Sistema de Radar"
...
-- build static menus
local static_menus = {}
SetTimeout(30000,function() -- wait for vRP.addStaticMenuChoices calls, aumentado para 30s
      ...
      if menu and smenu then
        ...
        local function smenu_leave()
          vRP.closeMenu(source)
    		...
    		if smenu.blipid ~= nil then
          vRPclient.addBlip(source,{x,y,z,smenu.blipid,smenu.blipcolor,smenu.title})
    		end
        vRPclient.addMarker(source,{x,y,z,1.5,1.5,0.7,255,226,0,125,150})
