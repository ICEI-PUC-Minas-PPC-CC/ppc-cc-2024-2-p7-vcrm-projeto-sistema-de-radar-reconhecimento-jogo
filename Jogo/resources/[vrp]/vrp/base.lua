...
AddEventHandler("queue:playerConnecting",function(source, ids, name, setKickReason, deferrals)
  ...
  if ids ~= nil and #ids > 0 then
    deferrals.update("Checando Identificadores (Contas Steam e Rockstar)...")
    vRP.getUserIdByIdentifiers(ids, function(user_id)
      ...
      if user_id ~= nil then -- check user validity 
        deferrals.update("Checando Banimento...")
        vRP.isBanned(user_id, function(banned)
          if not banned then
            deferrals.update("Checando Whitelist...")
            vRP.isWhitelisted(user_id, function(whitelisted)
              if not config.whitelist or whitelisted then
      			    if playerCount >= cv then
                  deferrals.done('Ops! O servidor está cheio! (' .. tostring(cv) .. ' players).')
        				else
        					...
        					if vRP.rusers[user_id] == nil then -- not present on the server, init
        					  ...
        					  vRP.getUData(user_id, "vRP:datatable", function(sdata)
          						...
          						deferrals.update("Carregando Último Login...")
              ...
              print(""..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: not whitelisted (user_id = "..user_id..")")
      				for i = 1, 2 do
      				  deferrals.update("Para entrar, acesse nosso Discord e solicite liberação do seu ID: "..user_id.."")
                Citizen.Wait(500)
      				end
              deferrals.done("Para entrar, acesse nosso Discord e solicite liberação do seu ID: "..user_id.."")
      				TriggerEvent("queue:playerConnectingRemoveQueues", ids)
            end
          end)
          else
            print(""..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: banned (user_id = "..user_id..")")
            deferrals.done("Banido (ID: "..user_id..").")
            TriggerEvent("queue:playerConnectingRemoveQueues", ids)
          end
        end)
      else
        print(""..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: identification error - Acesse nosso Discord: discord.gg/YsDpQ5H")
        deferrals.done("Identification Error. (Steam, Rockstar)")
        TriggerEvent("queue:playerConnectingRemoveQueues", ids)
      end
    end)
  else
    print(""..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: missing identifiers - Acesse nosso Discord: discord.gg/YsDpQ5H")
    deferrals.done("Missing identifiers. (Steam, Rockstar)")
    TriggerEvent("queue:playerConnectingRemoveQueues", ids)
  end
  Debug.pend()
end)
...
--[[AddEventHandler("playerDropped",function(reason)
  ...
  if user_id ~= nil then
    ...
    print(""..endpoint.." disconnected (user_id = "..user_id..")")
